...
- `face_tomb_walkthrough_v12_dynamic_media.php`
  - Latest variant: server-side scans `/AIFACES/GLASS` and `/AIFACES/HALLWAY` via PHP, feeds both into the map, and adds a red teleport cube in the teleporter room for hallway-gallery access.
...
14. **PHP-driven dynamic media pass**
   - `face_tomb_walkthrough_v12_dynamic_media.php` binds gallery content to live directory scans (`GLASS` + `HALLWAY`) and exposes both spaces through in-map teleporters.
